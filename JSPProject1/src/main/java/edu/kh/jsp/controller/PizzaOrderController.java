...
public class PizzaOrderController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		pizzaList.add( new Pizza("페퍼로니 피자", 10000) );
		pizzaList.add( new Pizza("포테이토 피자", 9000) );
		
		
		String pizza = req.getParameter("pizza");	// 1, 2, 3 중 하나 (value 값)
		...
		// 피자 종류 파악
		// -> pizza 는 "1"처럼 문자열 -> 정수로 형변환 (parsing) 필요
		...
		// price 에 amount 값 곱하기
		price *= Integer.parseInt(amount);
		
		// req 에 pizzaName, price 를 속성으로 추가(세팅)
