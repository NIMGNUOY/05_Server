...
public class ExampleController1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// Parameter : 매개변수 == 다른 곳의 값을 전달받아 올때 사용
		
		// req.getParameter("name 속성값");
		// -> 요청시 전달된 데이터 중 name 속성값이 일치하는 데이터의 value 를 얻어와
		//	 String 형태로 반환
		...
		// 서버에서 클라이언트로 응답하는 방법은 응답페이지 만들어 보냄
		PrintWriter out = resp.getWriter();		// 서버에서 클라이언트에게 쓰는 것(출력)
