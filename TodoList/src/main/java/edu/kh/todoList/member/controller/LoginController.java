	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if(loginMember != null) {	// 로그인 성공
				...
				// 5-1. session 만료 시간 지정(초 단위)
				session.setMaxInactiveInterval(60 * 60);  // 세션생성 후 1시간으로 변경
				...
				// forward : 요청 처리 후 자체적인 화면(위임받은 jsp 포함)이 존재하여 이를 이용해서 응답
				//			(위임 시 request, response 객체를 함께 위임)
				
				// redirect : 요청 처리 후 자체적인 화면이 없어서 화면이 있는 다른 요청을 다시 호출(재요청)
				...
				// 로그인 실패 메시지 띄우기
				// request 는 redirect 시 폐기됨
				// -> redirect 요청 후 응답페이지에서 request 에 담긴 속성은 사용 불가
				// -> session 에 속성 세팅하면 재요청 페이지에서도 사용 가능!
				session.setAttribute("message", "아이디 또는 비밀번호 불일치");
				
				// 이전 페이지로 redirect (request, redirect 폐기됨)
